...
def test_committed_status_is_derived_and_pending_is_not_pass() -> None:
    ...
    assert strata["REGULATORY"]["pass"] == 39
    assert strata["REGULATORY"]["pending"] == 2
    assert strata["REGULATORY"]["maximum_provisional_buffer"] == 11
    ...
    assert strata["CONFERENCE_OTHER"]["pass"] == 15
    assert strata["CONFERENCE_OTHER"]["pending"] == 0
    assert strata["CONFERENCE_OTHER"]["maximum_provisional_buffer"] == 0
